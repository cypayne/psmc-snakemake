...
gq_cutoff,dpcutoff,mq_cutoff,qd_cutoff,fs_cutoff,sor_cutoff,mqrs_cutoff,readpos_cutoff,indel_window = list(map(float,sys.argv[3:]))
	...
	for line in f1:
		...
		#If heterozygous, add IUPAC base to .insnp The USE_RGQ qualification was added on 20160511  --AES 
			...
			if USE_RGQ==True: #20160511 --AES
				...
				if max(needs_greater_than - numpy.array(list(map(float,[qual_dict[x] for x in ["GQ","DP"]]+[info_dict[x] for x in ["MQ","QD","MQRankSum","ReadPosRankSum"]]))))<=0 and max(numpy.array(list(map(float,[info_dict["FS"],info_dict["SOR"]])))-needs_less_than)<=0:    
		...
		#If homozygous variant site does not pass thresholds, mask with 'N'. Otherwise, it passes, and gets replaced with the ALT allele  	
			...
			if USE_RGQ==True:
				...
				if max(needs_greater_than[0:4] - numpy.array(list(map(float,[qual_dict[x] for x in ["GQ","DP"]]+[info_dict[x] for x in ["MQ","QD"]]))))>=0 or max(numpy.array(list(map(float,[info_dict["FS"],info_dict["SOR"]])))-needs_less_than)>=0:       
					variantout.write("\t".join([chr,str(pos),cols[3],"N"])+'\n')		
				else:
